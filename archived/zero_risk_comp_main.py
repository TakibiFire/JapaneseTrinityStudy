"""
オルカンと無リスク資産（固定利回り）の組み合わせによる4%ルールの生存確率への影響を比較するシミュレーション。
"""
...
def main():
  ...
  # 無リスク資産の定義
  rf_3 = ZeroRiskAsset(name="無リスク資産(3%)", yield_rate=0.03)
  rf_4 = ZeroRiskAsset(name="無リスク資産(4%)", yield_rate=0.04)

  # ---------------------------------------------------------------------------
  # 2. 戦略(Plan)の定義
  # ---------------------------------------------------------------------------
  # (株比率, 無リスク資産オブジェクト) のリスト
  stock_ratios = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3]
  rf_assets = [rf_4]  # rf_3
  ...
  for ratio in stock_ratios:
    for rf in rf_assets:
      strategies.append(
          Strategy(name=f"オルカン {round(ratio*100)}%",
                   initial_money=10000,
                   initial_loan=0,
                   yearly_loan_interest=0,
                   initial_asset_ratio={
  ...
  # 4. 可視化と保存
  # ---------------------------------------------------------------------------
  survival_image_file = 'docs/imgs/zero_risk_comp_survival.svg'
  distribution_image_file = 'docs/imgs/zero_risk_comp_distribution.svg'
  visualize_and_save(results=results,
                     html_file='temp/zero_risk_comp_result.html',
                     survival_image_file=survival_image_file,
                     distribution_image_file=distribution_image_file,
                     title='無リスク資産の利回りによる生存確率の比較',
                     summary_title='無リスク資産の影響サマリー（1,000回試行）',
  ...
  try:
    ...
    pattern = r'(<!--<zero_risk_comp_main\.py>-->).*?(<!--</zero_risk_comp_main\.py>-->)'
    if re.search(pattern, content, re.DOTALL):
      ...
      print(
          f"\033[91mWarning: Placeholder <zero_risk_comp_main.py> not found in {report_file}\033[0m",
          file=sys.stderr)
