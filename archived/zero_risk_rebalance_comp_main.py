"""
無リスク資産（固定利回り）との組み合わせにおける、リバランスの有無が4%ルールの生存確率へ与える影響を比較するシミュレーション。
"""
...
def main():
  ...
  for ratio in stock_ratios:
    # 1. No Rebalance
    strategies.append(
        Strategy(name=f"オルカン {round(ratio*100)}% (リバランスなし)",
                 initial_money=10000,
                 initial_loan=0,
                 yearly_loan_interest=0,
                 initial_asset_ratio={
    ...
    # 2. Annual Rebalance
    strategies.append(
        Strategy(name=f"オルカン {round(ratio*100)}% (毎年リバランス)",
                 initial_money=10000,
                 initial_loan=0,
                 yearly_loan_interest=0,
                 initial_asset_ratio={
  ...
  # 4. 可視化と保存
  # ---------------------------------------------------------------------------
  survival_image_file = 'docs/imgs/zero_risk_rebalance_comp_survival.svg'
  distribution_image_file = 'docs/imgs/zero_risk_rebalance_comp_distribution.svg'
  visualize_and_save(results=results,
                     html_file='temp/zero_risk_rebalance_comp_result.html',
                     survival_image_file=survival_image_file,
                     distribution_image_file=distribution_image_file,
                     title='無リスク資産併用時のリバランスの影響',
  ...
  try:
    ...
    pattern = r'(<!--<zero_risk_rebalance_comp_main\.py>-->).*?(<!--</zero_risk_rebalance_comp_main\.py>-->)'
    if re.search(pattern, content, re.DOTALL):
      ...
      print(
          f"\033[91mWarning: Placeholder <zero_risk_rebalance_comp_main.py> not found in {report_file}\033[0m",
          file=sys.stderr)
