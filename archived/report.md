...
例えばオルカンを初年度に1億円購入したとして50年運用したとします。仮に年リターンを7%、年ボラティリティを0%、11%、13%、15%、17%と変えた時の資産価値は以下のようになります。
...
ちなみに直近の動きを見ると、基準年である2020年を100とした場合、2025年には「112.0」まで上昇しています。つまり、たった5年間で私たちが普段買っているものの価格が平均して12%も上がったということであり、短期間でも物価変動が家計に与える影響は小さくありません。
...
本シミュレーションでは、この税金計算を以下のようにモデル化しています。
...
上の破産確率グラフを見ると 2. と 4. で少し乖離しているのが見えますが、それはやはり譲渡所得税の影響がシミュレーションの前半では薄いためです。ただ、譲渡所得をきちんとシミュレーションするツールは少ないので、近似したいのであれば「年出費を 11.5% 増やす」というのが一つの策かと思います。
...
信託報酬が1%の場合、30年破産確率は0%の時の28.6%から39.2%へと急激に悪化します。50年破産確率に至っては68.2%と、およそ7割の確率で資金が底を突く結果となっています。さらに信託報酬が「2%」の場合は、30年で半数以上（52.8%）が破産し、50年後には80.5%が破産します。わずか数パーセントの手数料の違いが、長期的な生存確率を大きく左右します。

上位25%の幸運なケースで見ても、信託報酬0%なら50年後に6.7億円残るはずの資産が、信託報酬1%では1.7億円、信託報酬2%ではゼロ（破産）になっています。手数料は確実に発生するマイナスのリターンであり、長期投資においては資産の寿命を縮める大きな要因となります。

したがって、長期の取り崩し戦略においては、信託報酬を低く抑えることが極めて重要です。手数料の高い商品を選ぶことは、資産寿命を維持する上で大きなリスク要因となることがわかります。
...
しかし、一般的に資産運用では「定率リバランス（例えば常に 80:20 の比率を保つように売買する）」が推奨されます。

このリバランスが、0%金利の現金と組み合わせた時にどう機能するかをシミュレーションしてみましょう。基準として「オルカン 100%」も並べています。
...
「オルカン 50%」に至っては、リバランスなしなら破産確率は 72.0% ですが、リバランスをすると 95.5% と、ほぼ確実に破産します。
...
シミュレーションはオルカン : 無リスク資産の割合のみを変えます。
1. 100% : 0%
1. 90% : 10%
1. 80% : 20%
1. 70% : 30%
...
1. **売却する順番によって有利な期間が全く異なる**:
   * **短・中期の安全重視（20〜30年の生存確率を上げたい）**: 「株式を優先して売る」方が圧倒的に安全です。運用初期の暴落時のクッションである無リスク資産を温存しておくことで、致命傷を避けることができます（例えば80%:20%の20年破産確率は、株優先8.6%に対し無リスク優先は12.3%まで悪化します）。
   * **長期の成長重視（40〜50年の生存確率を上げたい）**: 「無リスク資産を優先して売る」方が有利です。リターンの低い資産から順に消費し、期待リターンの高い株式をできるだけ長く運用し続ける戦略が、超長期の資産寿命を延ばすのに貢献します。
2. **80%:20% は 50年生存に影響せず、短中期の生存率を劇的に上げる**:
   「オルカン 80% / 無リスク資産 20%」の配分で**株優先**で売却すると、50年後の破産確率を 100%株とさほど変えずに（100%株62.5% → 80%株68.3%）、20〜30年といったリタイア中期の生存確率を大幅に改善させます。
   * **20年破産確率**: 14.3% (100%株) → 8.6% (80/20・株優先)
   * **30年破産確率**: 40.0% (100%株) → 40.7% (80/20・株優先)
   （※10年破産確率で見ると 0.3% → 0.0% とほぼ完璧に防御できています）

つまり、**少量の無リスク資産を持ち、基本的には株から取り崩して無リスク資産を温存しておく**ことが、リタイア初期の破綻リスクを回避するための、極めて合理的かつ有効な戦術となります。

### 生存確率を上げる戦略: 無リスク資産のリバランスを考える
...
### 生存確率を上げる戦略: ダイナミック・スペンディング（動的支出）戦略

「4%ルールっておかしいよね」という話は海外でもされていて、それに対する解決策としてヴァンガード社が 2021年に提案した方法が「ダイナミック・スペンディング戦略」です ([pdf](https://www.vanguardmexico.com/content/dam/intl/americas/documents/mexico/en/fuel-for-the-fire.pdf))。
...
具体例でいきましょう。仮にある年の支出が400万円だとします。その場合、次の年の支出の上限は 420万円 (= 400 × 1.05)、下限は394万円 (= 400 × 0.985) となります。運用の結果資産が変動しますが、次の年の最初の資産によって次の年の支出を決めます。

* 9,600万円の場合 → 目標額は 384万円 (= 9,600 × 4%)。下限を下回ったので、支出は384万円とする。
* 10,400万円の場合 → 目標額は 416万円 (= 10,400 × 4%)。下限〜上限の範囲内なので、支出は416万円とする。
* 10,8000万円の場合 → 目標額は 432万円 (= 10,800 × 4%)。上限を超えたので、支出は420万円とする。

という流れとなります。これによって、運用が好調なときは支出を伸ばし、不調なときは無理ない範囲で支出を減らしていくことを目指します。
...
に固定したうえで、以下の場合を比較していきましょう。

1. 400万円支出、インフレ率 2%
1. ダイナミック・スペンディング (資産の4%目標、上限 5%、下限 -1.5%)
1. ダイナミック・スペンディング (資産の4%目標、上限 2%、下限 -2%)
1. ダイナミック・スペンディング (資産の4%目標、上限 2%、下限 0%)

<!--<dynamic_spending_comp_main.py>-->

|                          |   下位1% (だいぶ運が悪い) |   下位10% (運が悪い) |   下位25% (やや不運) |   中央値 (普通) |   上位25% (やや幸運) |   上位10% (運が良い) |   10年破産確率 (%) |   20年破産確率 (%) |   30年破産確率 (%) |   40年破産確率 (%) |   50年破産確率 (%) |
|:-------------------------|-----------------:|---------------:|---------------:|-----------:|---------------:|---------------:|--------------:|--------------:|--------------:|--------------:|--------------:|
| 1. 支出一定 (400万円, インフレ2%)  |            0.0億円 |          0.0億円 |          0.0億円 |      0.0億円 |          2.4億円 |         12.0億円 |          0.0% |          9.3% |         36.6% |         53.4% |         63.8% |
| 2. 動的支出 (上限+5%, 下限-1.5%) |            0.0億円 |          0.0億円 |          0.0億円 |      0.7億円 |          2.4億円 |          7.2億円 |          0.0% |          2.3% |         10.1% |         21.4% |         34.1% |
| 3. 動的支出 (上限+2%, 下限-2%)   |            0.0億円 |          0.0億円 |          0.2億円 |      1.6億円 |          4.9億円 |         14.6億円 |          0.0% |          1.9% |          6.8% |         13.7% |         19.5% |
| 4. 動的支出 (上限+2%, 下限0%)    |            0.0億円 |          0.0億円 |          0.0億円 |      0.6億円 |          3.9億円 |         14.0億円 |          0.0% |          4.2% |         16.2% |         30.8% |         41.6% |

<!--</dynamic_spending_comp_main.py>-->

50年後の資産の分布は以下のようになります。

<img src="imgs/dynamic_spending_comp_distribution.svg"/>

生存確率の詳細グラフは以下の通り。

<img src="imgs/dynamic_spending_comp_survival.svg" />

ヴァンガードが提案する「動的支出（上限+5%, 下限-1.5%）」(オレンジ)は、ベースラインの「支出一定（4%ルール）」(紺色)と比べて、生存確率が劇的に改善しますね！

* **30年破産確率**: 36.6% から 10.1% へと大幅に減少。
* **50年破産確率**: 63.8% から 34.1% へと半減。

これは、運用初期の暴落時に無理な取り崩しを控える「下限」と、運用好調時に生活水準を上げすぎない「上限」が、資産寿命の延命に極めて有効に働いていることを示しています。特に、「下限」の存在により、収益配列リスクに対する強力な防御策となっています。

さらに厳しい「動的支出（上限+2%, 下限-2%）」や「動的支出（上限+2%, 下限0%）」では、破産確率がさらに下がります。市場の変動に合わせて生活費を調整できることが、生存確率を上げる有効な手段であることがわかりますね。

もう一つ、このアイデアがなぜ有効かと言うと「インフレ率を無視しているから」とも言えます。また、この戦略は「世の中の物価が2%上昇しているにも関わらず自分の出費を1.5%減らす」といった、実質的な支出削減を前提としています。生活水準を維持したい中で支出を削るにはそれなりの覚悟が必要ですが、それが可能な場合には生存確率は顕著に向上します。

### 生存確率を上げる戦略: 高齢になると支出が減るという話
...
* **30代・40代のアーリーリタイアは非常に過酷**: 「支出一定（400万円）」のベースラインと比較して、「30歳からの50年間」や「40歳からの50年間」は破産確率が顕著に悪化しています（例えば30歳開始の30年破産確率は 54.7% にも達します）。これは、リタイア直後から50代半ばにかけて生活費の「増加トレンド」が続くためです。運用初期に一番資産を減らしたくない時期に、出費の増加が追い打ちをかけることになり、リタイア直後の資産が不安定な時期に支出が増えることは、長期的な資産運用にとって大きなマイナスとなり、当初の取り崩し計画を維持することが困難になります。
...
ちなみに、現実の支出や収入はもっと複雑で、以下のことを考えるべきです。

* **子供の教育費**: 特に大学の4年間は、国公立か私立か、自宅通学か下宿かによって数百万円単位で支出が跳ね上がります。
* **住宅や医療のまとまった出費**: 持ち家の場合は10〜20年周期で外壁塗装や水回りの修繕などのリフォーム費用が発生します。また高齢期には、高額療養費制度の適用外となる差額ベッド代や、介護施設の入居一時金などが必要になることがあります。
* **社会保険料の増加リスク**: 資産運用による利益を確定申告した場合、国民健康保険料などの算定基準となる「所得」に加算され、翌年の保険料が数十万円単位で跳ね上がることがあります（源泉徴収ありの特定口座で申告不要を選択すれば防げます）。
* **公的年金**: 65歳などから受給が始まる国民年金や厚生年金は終身で受け取れるため、高齢期の取り崩し額を大きく減らす支えになります。
* **退職金やiDeCo**: リタイア時点や60歳以降に、まとまった一時金や年金形式で受け取ることができます。
* **労働所得**: 完全にリタイアせず、アルバイトなどで少額でも収入を得る（サイドFIRE）と、資産の取り崩しペースを大幅に落とすことができます。
* **相続**: 予測は難しいですが、親世代からの資産承継が後半の資産寿命を延ばすケースもあります。

これらは個人のライフスタイル、家族構成、シミュレーションの最初の資産額などに大きく依存するため、今回のシミュレーションでは取り扱いません。

ただし、このうち年金はある程度はあてにしてもいい収入だということは覚えておいて下さい。
...
* 利率4%の無リスク資産を混ぜると、30年目までの生存確率は上がるけど50年生存確率は下がる
* 株式を優先して売却し無リスク資産を温存すると、中短期（20〜30年）の破綻リスクを劇的に抑えられる
* 利率の良い無リスク資産との定率リバランスは、30年程度の生存確率向上に寄与する
* 暴落時に支出を切り詰める「ダイナミック・スペンディング（動的支出）」を取り入れると、生存確率は大幅に改善する
* 30〜40代のアーリーリタイアは出費増加期と重なるためシミュレーション以上に厳しく、50代以降は加齢による支出減のおかげでシミュレーションより楽観的になれる

これらから見えてくる最も重要な結論は、**「目指す生存期間を30年にするか、50年にするかで取るべき戦略が大きく変わる」**ということです。30年の生存（運用初期の暴落回避）を優先するなら無リスク資産を厚めに持つなどの「安定重視」が正解になりますが、50年という超長期の生存を目指すなら、株式の高リターンを最大限活かす「成長重視」の戦略が必要になります。

### 何%ルールが正解なのか

今まで4%の支出にこだわっていましたが、ついにこの数字を変えていこうと思います。

オルカンと無リスク資産で運用するとして、その最適な比率は

* 初年度の出費が総資産の何%か
* 何年目の生存を目標とするか

によることが見えてきたのでシミュレーションで求めてみます。

なお、以下のシミュレーションの議論では、話をシンプルにするために年齢ごとの支出変動は一旦忘れることにします。「30〜40代で開始するなら結果より少し厳しく、50代以降なら結果より少し楽観的でいられる」とだけ覚えておいてください。

また、ダイナミック・スペンディングに関しても、ここからのシミュレーションには組み込みません。「いざという時に支出を我慢できる柔軟性があれば、さらに楽観的でいられる」という強力なセーフティネットとして心に留めておきましょう。

シミュレーションの固定条件は以下の通りです。
* インフレ率: 2%
* 無リスク資産利回り: 4% (譲渡所得税源泉徴収あり)
* 譲渡所得税: 20.315% (売却時の利益に対して発生)
* リバランス: 1年ごと
* 売却順序: 1. 無リスク資産、2. 株式 (オルカン)。ただしリバランスを頻繁に行うのでこの影響は小さいとみなす

変更する条件:
* **支出率**: 5% (x20) ~ 2% (x50) まで様々
* **オルカン比率**: 100% ~ 0% の 5%刻み

これらを組み合わせた全てのパターンについて、1,000回のモンテカルロ・シミュレーションを行い、生存確率を算出しました。

その上で (支出率、目標年数) ごとに生存確率を最大にするオルカン比率を求めました。

<img src="imgs/optimal_orukan.png"/>

例えばあなたの現在の年出費が総資産の20倍 (5%支出)の場合、10年生存確率を最大にするオルカン比率は65%、35年生存確率を最大にするオルカン比率は95%です。

この詳細なデータは [Spreadsheet](https://docs.google.com/spreadsheets/d/1L-OzwAyeaFS1uQ-pt-KMLiLmFjXj4R4gzbapUjDI_1g/edit?usp=sharing) に保存されています。

表の傾向から、無リスク資産（実質利回り 約3.18%）だけを切り崩していったときに資産が底をつく理論上の年数（**「資産寿命」** と呼びます）を境に、戦略が大きく二分されることがわかります。
...
1. **目標年数が資産寿命より短い場合**: 運用初期の暴落さえ回避できれば無リスク資産だけで逃げ切れる可能性が高いため、無リスク資産の割合を多めにしてリスクを最小限に抑えるのが正解になります。
2. **目標年数が資産寿命より長い場合**: 無リスク資産だけでは確実に枯渇してしまうため、逆に株式（オルカン）の比率を高めてリターンを追求する必要があります。

このデータを元に、他の値でもオルカン比率を予測する近似式（ルール・オブ・サム）とロジックを作ってみました。詳細な計算やJavascriptコードは以下の詳細タブに格納しておきます。

<details>
<summary>(プログラマー向け) Javascript Code と近似式の求め方</summary>
...
支出率 ($S$) と目標とする経過年数 ($N$) に応じて、生存確率を最大化する「最適オルカン比率」を予測する近似式（ルール・オブ・サム）は以下のようになります。理論上の資産寿命を $N_{ruin}$ とします。

$$n = N / 50$$
$$m = (N - N_{ruin}) / 50 \quad (\text{Region 2 のみ})$$

**【Region 1: $N \le N_{ruin}$ の場合】**
$$g(S, n) = -0.7967 -0.3815 \ln(n \cdot S) + \frac{0.1160}{n} -0.2093 \ln(S) -1.4521 e^S$$

**【Region 2: $N > N_{ruin}$ の場合】**
$$h(S, m) = +0.7746 +0.1809 \ln(m) - \frac{0.0317}{S} + 0.9758 e^S + 0.0075 \frac{n}{S}$$
...
* **「ずっと一定比率」は最適ではない**: 多くの投資本では「株80:債券20で固定」といったリバランスが推奨されますが、生存確率を極限まで高めるなら、目標とする時期が資産寿命の内側か外側かによって、比率をダイナミックに変えることが有効そうです。
* **若い時のリタイアほど高い株式比率が必要**: 50年以上の生存を目指す場合、必然的に目標年数が資産寿命を超える期間が長くなります。そのため、初期から高いオルカン比率を維持する必要があります。
* **高齢期の安心は「無リスク資産」で作る**: 残りの目標年数が資産寿命の範囲内に収まるような短・中期の計画（例えばあと15年〜20年持てば良いという状況）であれば、オルカン比率を大胆に下げて無リスク資産に寄せることで、市場の混乱に怯えることなく生活を維持できます。

## ダイナミックリバランス

以上のことから、(残りの目標年数, 初年度年支出割合) の値から最適なリバランス値に変えていくことが効果がありそうだとわかりました。

ちなみにリスク資産の割合を (100-年齢)% や (110-年齢)% にするアイデアはよく知られています。

参考: [オルカンの次は？　GPIFと「110－年齢」の法則で考える資産配分 | SBI 証券](https://go.sbisec.co.jp/media/report/bond_now/bond_now_251127.html)

このアイデアとも比較しながら実際にシミュレーションしてみましょう。
...
比較するのは以下の設定です
* 初年度の時の (目標年数, 初年度年支出割合) の値から得られる最適なリバランス値を変えずにずっとキープする
* 「ダイナミックリバランス」: 1年ごとに (残りの目標年数, 今年度年支出割合) の値から得られる最適なリバランス値に変更する
* 比較: (110 - 年齢) をオルカン比率にする。年齢は30, 40, 50, 60 を使用。

この比較を様々な (目標年数, 初年度年支出割合) で行い、目標年数後の生存確率を計算しました。

結果の一部は以下の通りです。

<!--
$ python3 dynamic_rebalance_comp_main.py

でグリッドサーチを開始。少し時間がかかる。 data/dynamic_rebalance_comp.csv を生成。

$ python3 analyze_dynamic_rebalance_main.py

で結果を data/dynamic_rebalance_summary.csv にまとめる。
-->

<img src="imgs/dynamic_rebalance.png"/>
<!-- NOTE: data/dynamic_rebalance_summary.csv と同じ内容 -->
