...
def plot_fx_with_fits(dates: List[datetime], prices: List[float], 
                      fits: Dict[str, Tuple[int, float]], output_path: str) -> None:
  ...
  for label, (start_idx, mu_annual) in fits.items():
    ...
    fit_prices = [start_price * ((1 + mu_monthly) ** i) for i in range(len(fit_dates))]
  ...
  chart = alt.Chart(df).mark_line().encode(
      ...
      color=alt.Color("Type:N", title="データ種類", 
                      scale=alt.Scale(
                          domain=["Actual FX Rate", "Fit 1986-", "Fit 2000-", "Fit 2013-"],
...
def main() -> None:
  ...
  # グラフ描画
  plot_fx_with_fits(all_dates, all_prices, fits_info, "imgs/fx.svg")
