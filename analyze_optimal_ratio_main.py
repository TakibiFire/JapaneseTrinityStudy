...
def main():
  ...
  if not os.path.exists(csv_path):
    print(f"Error: {csv_path} が見つかりません。先に withdrawal_rate_comp_main.py を実行してください。")
    return
  ...
  def get_features_for_piece(S: float, N_raw: float, N_ruin: float) -> Dict[str, float]:
    n = N_raw / 50.0  # 正規化
  ...
  def solve_piece(points, name, num_terms=4):
    ...
    # 各点において N_ruin を再計算
    X_dict_list = [get_features_for_piece(S, float(N), get_ruin_year(S)) for S, N, v in points]
    ...
    for step in range(num_terms):
        ...
        for f in feature_names:
            ...
            res = opt.minimize(loss_func, w0, args=(X_mat, Y_arr), method='L-BFGS-B')
            error = res.fun
  ...
  points_ratio_before, points_ratio_after = get_training_points(summary_df, filter_boundary=True)
  feat_ratio_before, w_ratio_before = solve_piece(points_ratio_before, "Ratio Region 1 (N <= N_ruin)")
  feat_ratio_after, w_ratio_after = solve_piece(points_ratio_after, "Ratio Region 2 (N > N_ruin)")

  # --- 生存確率のモデル ---
  print("\n--- 最大生存確率の Piecewise モデル計算 ---")
  # 生存確率の場合、100% (1.0) の地点は予測において支配的なので、境界フィルタリングを適用
  points_prob_before, points_prob_after = get_training_points(prob_df, filter_boundary=True)
  
  feat_prob_before, w_prob_before = solve_piece(points_prob_before, "Prob Region 1 (N <= N_ruin)")
  feat_prob_after, w_prob_after = solve_piece(points_prob_after, "Prob Region 2 (N > N_ruin)")
  ...
  def format_formula(fs, ws):
    if fs is None: return "1.0000 (Fixed)"
  ...
  # サンプル比較
  print("\n--- サンプル比較 (実測値 vs 予測値) ---")
  sample_points = [(0.025, 20), (0.03, 30), (0.04, 15), (0.05, 30), (0.066667, 20)]
  print(f"{'S':>8} | {'N':>3} | {'Ratio実':>7} | {'Ratio予':>7} | {'Prob実':>7} | {'Prob予':>7}")
  print("-" * 70)
  for S, N in sample_points:
    if S not in summary_df.index: continue
    r_act = summary_df.loc[S, str(N)]
    r_pre = predict_ratio(S, float(N))
    p_act = prob_df.loc[S, str(N)]
    p_pre = predict_prob(S, float(N))
    print(f"{S:8.3f} | {N:3d} | {r_act:7.2f} | {r_pre:7.2f} | {p_act:7.2f} | {p_pre:7.2f}")
