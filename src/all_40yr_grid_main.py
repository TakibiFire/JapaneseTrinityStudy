...
- インフレ: AR(12) 粘着性モデル
- 初年度支出ベースライン: 統計データに基づく40歳時の平均支出
- 税率: 20.315%
- 年金保険料: 40-60歳まで国民年金保険料を支払い (1人: 21.5万/年)
...
def get_optimal_pension_setup(
    base_spend_40_retired: float, pension_premium_annual: float
) -> Tuple[Setup, int, List[Tuple[int, float, float]]]:
  """
  optimal-pension 実験設定を生成する。

  Args:
    base_spend_40_retired: 40歳時の基本支出額（年金保険料除く、万円）。
...
def get_pen70_formula_setup(
  ...
  spend_multipliers = [0.75, 1.0, 1.5, 2.0, 3.0]
  spending_rules = [
      1.5, 1.66, 1.8, 1.89, 2.0, 2.13, 2.22, 2.38, 2.5, 2.8, 3.0, 3.33, 3.66,
      4.0, 4.33, 4.66, 5.0
  ]
...
def main():
  ...
  # 初年度支出ベースライン (40歳、年金保険料除く)
  base_spend_40_retired = get_annual_retired_spending_values(
