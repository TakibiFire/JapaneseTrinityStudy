...
def main() -> None:
  ...
  # 各年・各アセットごとに計算
  melted = rets_df.melt(id_vars='Year', var_name='Asset', value_name='Return').dropna()
  # groupbyの結果を明示的に扱う
  ...
  sp500_30y = results_df[(results_df['Asset'] == 'S&P 500') & (results_df['Year'] >= 1996) & (results_df['Year'] <= 2025)]
  acwi_18y = results_df[(results_df['Asset'] == 'ACWI (Orukan)') & (results_df['Year'] >= 2008) & (results_df['Year'] <= 2025)]
  ...
  # 色の設定
  colors = alt.Scale(domain=['S&P 500', 'ACWI (Orukan)'], range=['#1f77b4', '#ff7f0e'])
  
  # 共通のベースチャート
  base = alt.Chart(plot_df).encode(
      x=alt.X('Year:O', title='年'),
      color=alt.Color('Asset:N', scale=colors, title='アセット',
  ...
  return_chart = base.mark_line(point=True).encode(
      y=alt.Y('Annual Arithmetic Return:Q', title='年率算術リターン', axis=alt.Axis(format='%')),
      tooltip=['Year', 'Asset', alt.Tooltip('Annual Arithmetic Return:Q', format='.2%')]
  ).properties(
      title='年率算術リターンの推移'
  )
  ...
  vol_chart = base.mark_line(point=True).encode(
      y=alt.Y('Annual Arithmetic Volatility:Q', title='年率算術ボラティリティ', axis=alt.Axis(format='%')),
      tooltip=['Year', 'Asset', alt.Tooltip('Annual Arithmetic Volatility:Q', format='.2%')]
  ).properties(
      title='年率算術ボラティリティの推移'
  )
  ...
  charts = [
      (return_chart, 'sp500_acwi_annual_return.svg'),
