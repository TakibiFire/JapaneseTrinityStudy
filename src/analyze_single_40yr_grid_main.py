"""
data/single_40yr/ の結果を分析・可ビジュアル化するスクリプト（単身世帯版）。
...
# 設定
IMG_DIR = "docs/imgs/single_40yr"
DATA_OUT_DIR = "docs/data/single_40yr"
NUM_YEARS = 55
START_AGE = 40
...
def run_optimal_pension_analysis(df_all: pd.DataFrame, target_year: str):
  ...
  create_pension_survival_curve(df_all,
                                multiplier=1.5,
                                rule=2.5,
                                title="受給開始年齢別 生存確率推移 (支出レベル1.5, 初年度支出率4.5%)",
                                output_path=os.path.join(
                                    IMG_DIR,
                                    "survival_curve_pension_m1_5_r2_5.svg"),
                                start_age=START_AGE,
                                num_years=NUM_YEARS)
...
def main():
  parser = argparse.ArgumentParser(
      description="40歳リタイア開始・95歳までの分析・可視化スクリプト（単身世帯版）。")
  ...
  for et in exp_types:
    et = et.strip()
    csv_path = f"data/single_40yr/{et}.csv"
