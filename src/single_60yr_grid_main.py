"""
60歳リタイア開始・95歳開始（94歳末）までの生存確率を分析するグリッドサーチスクリプト（単身世帯版）。
...
- インフレ: AR(12) 粘着性モデル
- 初年度支出ベースライン: 単身世帯の2025年統計データに基づく60歳時の平均支出
- 税率: 20.315%
...
def get_optimal_pension_setup(
  ...
  spending_rules = [3.0, 4.0, 5.0, 6.0, 7.0, 9.0, 11.0, 15.0, 20.0, 25.0]
  N_SIM = 2000
...
def main():
  ...
  parser = argparse.ArgumentParser(
      description="60歳リタイア開始・95歳開始（94歳末）までの生存確率を分析するグリッドサーチスクリプト（単身世帯版）。")
