...
def run_fitting_analysis(df: pd.DataFrame, target_col: str):
  ...
  feats = pd.DataFrame({
      ...
      "invX": 1.0/X_val, "invY": 1.0/Y_val, "invZ": 1.0/Z_val
  })
...
def run_stepwise_analysis(df: pd.DataFrame, target_col: str):
  ...
  for i in range(len(keys)):
    for j in range(i, len(keys)):
      k1, k2 = keys[i], keys[j]
      if (k1 == "X" and k2 == "invX") or (k1 == "Y" and k2 == "invY") or (k1 == "Z" and k2 == "invZ"):
        continue
  ...
  for step in range(1, 11):
    ...
    if best_feat:
      ...
      print(f"{step:4d} | {best_feat:<20} | {best_r2:8.4f} | {improvement:+8.4f}")
...
def create_heatmap(df: pd.DataFrame, target_col: str, title: str, x_col: str, x_title: str,
                   y_col: str, y_title: str, output_name: str, 
                   x_sort: List = None, y_sort: List = None):
  """
  ヒートマップを作成して保存する。
  """
  plot_df = df.copy()
  ...
  if y_sort: pivot = pivot.reindex(index=y_sort)
  if x_sort: pivot = pivot.reindex(columns=x_sort)
...
def main():
  ...
  # ラベルの日本語化
  df["threshold_label"] = df["threshold_x"].map(lambda x: f"x{1.0/x:.1f}" if 1.0/x != int(1.0/x) else f"x{int(1.0/x)}")
  ...
  for year_target in ["30", "50"]:
    ...
    # 3. 可視化
    current_y_order = [y for y in y_order if int(y.replace("年", "")) <= int(year_target)]
    ...
    create_heatmap(
        ...
        x_col="income_label", x_title="労働収入レベル (支出比)",
        y_col="year_label", y_title="最大労働期間",
        output_name=f"heatmap_yz_at_x10_{year_target}yr.svg",
        x_sort=z_order, y_sort=current_y_order[::-1]
    )
    ...
    if not df_y20.empty:
      create_heatmap(
          ...
          x_col="threshold_label", x_title="トリガー閾値 (資産/支出)",
          y_col="income_label", y_title="労働収入レベル (支出比)",
          output_name=f"heatmap_xz_at_y20_{year_target}yr.svg",
          x_sort=x_order, y_sort=z_order[::-1]
      )
