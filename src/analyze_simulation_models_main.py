...
def main():
  ...
  print(
      f"SP500 All-Time: {sp500_simple_all.index.min().strftime('%Y-%m')} to {sp500_simple_all.index.max().strftime('%Y-%m')}"
  )

  start_30y = '1996-01-01'
  sp500_simple_30y = sp500_simple[(sp500_simple.index >= start_30y) &
                                  (sp500_simple.index <= end_date)]
  sp500_log_30y = sp500_log[(sp500_log.index >= start_30y) &
                            (sp500_log.index <= end_date)]
  print(
      f"SP500 Recent 30y: {sp500_simple_30y.index.min().strftime('%Y-%m')} to {sp500_simple_30y.index.max().strftime('%Y-%m')}"
  )
  ...
  mu_y1, std_y1 = asset_model.simulate_annual_stats_simple(
      stats.norm, (res_sp1['mu'], res_sp1['std']))
  print(
      f"SP500-1 (Simple Normal, All): mu_M={res_sp1['mu']:.6f}, sigma_M={res_sp1['std']:.6f}, mu_Y={mu_y1:.6f}, sigma_Y={std_y1:.6f}, BIC={res_sp1['bic']:.2f}"
  )

  # SP500-2: Log-Normal (All)
  res_sp2 = asset_model.fit_normal_log(sp500_log_all)
  mu_y2, std_y2 = asset_model.simulate_annual_stats_log(
      stats.norm, (res_sp2['mu'], res_sp2['std']))
  print(
      f"SP500-2 (Log-Normal, All): mu_M={res_sp2['mu']:.6f}, sigma_M={res_sp2['std']:.6f}, mu_Y={mu_y2:.6f}, sigma_Y={std_y2:.6f}, BIC={res_sp2['bic']:.2f}"
  )

  # SP500-3: Best Fit (All)
  res_sp3 = asset_model.find_best_distribution(sp500_log_all, top_n=1)[0]
  dist3 = getattr(stats, res_sp3['name'])
  mu_y3, std_y3 = asset_model.simulate_annual_stats_log(dist3,
                                                        res_sp3['params'])
  print(
      f"SP500-3 (Best Fit, All): dist={res_sp3['name']}, params={res_sp3['params']}, mu_Y={mu_y3:.6f}, sigma_Y={std_y3:.6f}, BIC={res_sp3['bic']:.2f}"
  )
  ...
  if b < 0:
    ...
    print(
        f"SP500-4 (MR-GBM, Monthly): theta={theta:.6f}, mu={mu:.6f}, sigma={sigma:.6f}"
    )
  else:
  ...
  mu_y5, std_y5 = asset_model.simulate_annual_stats_log(
      stats.norm, (res_sp5['mu'], res_sp5['std']))
  print(
      f"SP500-5 (Log-Normal, 30y): mu_M={res_sp5['mu']:.6f}, sigma_M={res_sp5['std']:.6f}, mu_Y={mu_y5:.6f}, sigma_Y={std_y5:.6f}, BIC={res_sp5['bic']:.2f}"
  )

  # SP500-6: Best Fit (30y)
  res_sp6 = asset_model.find_best_distribution(sp500_log_30y, top_n=1)[0]
  dist6 = getattr(stats, res_sp6['name'])
  mu_y6, std_y6 = asset_model.simulate_annual_stats_log(dist6,
                                                        res_sp6['params'])
  print(
      f"SP500-6 (Best Fit, 30y): dist={res_sp6['name']}, params={res_sp6['params']}, mu_Y={mu_y6:.6f}, sigma_Y={std_y6:.6f}, BIC={res_sp6['bic']:.2f}"
  )
  ...
  print(
      f"ACWI All-Time: {acwi_simple_all.index.min().strftime('%Y-%m')} to {acwi_simple_all.index.max().strftime('%Y-%m')}"
  )

  # ACWI-1: Simple Normal (All)
  res_ac1 = asset_model.fit_normal_simple(acwi_simple_all)
  mu_ya1, std_ya1 = asset_model.simulate_annual_stats_simple(
      stats.norm, (res_ac1['mu'], res_ac1['std']))
  print(
      f"ACWI-1 (Simple Normal, All): mu_M={res_ac1['mu']:.6f}, sigma_M={res_ac1['std']:.6f}, mu_Y={mu_ya1:.6f}, sigma_Y={std_ya1:.6f}, BIC={res_ac1['bic']:.2f}"
  )

  # ACWI-2: Log-Normal (All)
  res_ac2 = asset_model.fit_normal_log(acwi_log_all)
  mu_ya2, std_ya2 = asset_model.simulate_annual_stats_log(
      stats.norm, (res_ac2['mu'], res_ac2['std']))
  print(
      f"ACWI-2 (Log-Normal, All): mu_M={res_ac2['mu']:.6f}, sigma_M={res_ac2['std']:.6f}, mu_Y={mu_ya2:.6f}, sigma_Y={std_ya2:.6f}, BIC={res_ac2['bic']:.2f}"
  )

  # ACWI-3: Best Fit (All)
  res_ac3 = asset_model.find_best_distribution(acwi_log_all, top_n=1)[0]
  dista3 = getattr(stats, res_ac3['name'])
  mu_ya3, std_ya3 = asset_model.simulate_annual_stats_log(
      dista3, res_ac3['params'])
  print(
      f"ACWI-3 (Best Fit, All): dist={res_ac3['name']}, params={res_ac3['params']}, mu_Y={mu_ya3:.6f}, sigma_Y={std_ya3:.6f}, BIC={res_ac3['bic']:.2f}"
  )
  ...
  print(
      f"ACWI-APP-1 (Normal Noise): mu_M={res_app1['mu']:.6f}, sigma_M={res_app1['std']:.6f}, BIC={res_app1['bic']:.2f}"
  )

  # ACWI-APP-2: Best Fit to residuals
  res_app2 = asset_model.find_best_distribution(residuals, top_n=1)[0]
  print(
      f"ACWI-APP-2 (Best Fit Noise): dist={res_app2['name']}, params={res_app2['params']}, BIC={res_app2['bic']:.2f}"
  )
  ...
  print(
      f"2. ACWI-3 (Direct Best Fit, {res_ac3['name']}): LL = {res_ac3['loglik']:.2f}"
  )
  ...
  ll_induced = stats.norm.logpdf(acwi_log_all,
                                 loc=induced_mu,
                                 scale=induced_std).sum()
  print(
      f"3. ACWI-APP-1 + SP500-2 (Induced Marginal Normal): LL = {ll_induced:.2f}"
  )

  # Calculate Induced Marginal from ACWI-APP-1 + SP500-5
  induced_mu_30y = slope * res_sp5['mu'] + intercept + res_app1['mu']
  induced_std_30y = np.sqrt((slope**2) * (res_sp5['std']**2) +
                            (res_app1['std']**2))
  ll_induced_30y = stats.norm.logpdf(acwi_log_all,
                                     loc=induced_mu_30y,
                                     scale=induced_std_30y).sum()
  print(
      f"4. ACWI-APP-1 + SP500-5 (Induced Marginal Normal): LL = {ll_induced_30y:.2f}"
  )

  print(
      f"(Note: A higher Log-Likelihood (closer to 0) means the model is more likely to generate the actual ACWI data.)"
  )
