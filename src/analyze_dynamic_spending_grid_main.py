...
def print_representative_stats(df: pd.DataFrame, exp_name: str):
  ...
  for up, low, label in conditions:
    res = df[(np.isclose(u_vals, up)) &
             (np.isclose(l_vals, low))]
    if not res.empty:
      s30 = float(res["30"].values[0])
      s50 = float(res["50"].values[0])
      stats_data.append({"ラベル": label, "30年": f"{s30:.1%}", "50年": f"{s50:.1%}"})
...
def create_spend_distribution_charts(df: pd.DataFrame, exp_name: str):
  ...
  for y in target_years:
    ...
    rules_data = pd.DataFrame([
        {'x': initial_spend, 'label': '初期の生活水準 (100%)'},
        {'x': p25, 'label': '25パーセンタイル'},
        {'x': p50, 'label': '50パーセンタイル (中央値)'},
        {'x': p75, 'label': '75パーセンタイル'}
    ])
    
    rules = alt.Chart(rules_data).mark_rule(size=2).encode(
        x='x:Q',
        color=alt.Color('label:N', title='', scale=alt.Scale(
            domain=['初期の生活水準 (100%)', '25パーセンタイル', '50パーセンタイル (中央値)', '75パーセンタイル'],
            range=['red', 'blue', 'green', 'purple']
        )),
        strokeDash=alt.condition(
            alt.datum.label == '初期の生活水準 (100%)',
            alt.value([5, 5]),
            alt.value([0])
        )
    )
    ...
    final_chart = (hist + rules).properties(
        title=f"実質支出額（購買力）の分布: {y}年目 (上限{upper:.1%}, 下限{lower:.1%}) - 破産ケース {bankrupt_rate:.1%} を除外",
        width=600,
        height=300
    ).configure_legend(
        orient='top',
...
def process_experiment(exp_name: str):
  ...
  for target_year in target_years:
    ...
    output_path = os.path.join(output_dir, f"grid_heatmap_{exp_name}_{target_year}yr_survival.svg")
...
def main():
  ...
  parser = argparse.ArgumentParser(
      description="グリッドシミュレーションの結果を解析・可視化する")
