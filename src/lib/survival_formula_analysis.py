...
def run_survival_formula_analysis(df_survival: pd.DataFrame, target_year: str = "35") -> None:
  """
  ...
  for p in target_probs:
    ...
    for rule, spend, m in anchors:
      data_points.append({"p": p, "logit_p": np.log(p / (1 - p)), "M": m, "Spend": spend})
  ...
  def solve_logit_quad(m, spend):
    ...
    if 0.3 <= root1 <= 4.5: return root1
    if 0.3 <= root2 <= 4.5: return root2
    return root1
  ...
  cm, km, kc, cc = model_lin.coef_[0], model_lin.coef_[1], model_lin.coef_[2], model_lin.intercept_
  ...
  df_fit_f["p_quad"] = df_fit_f.apply(lambda r: solve_logit_quad(r["M"], r["Spend"]), axis=1)
  df_fit_f["p_lin"] = (df_fit_f["Spend"] - (cm * df_fit_f["M"] + cc)) / (km * df_fit_f["M"] + kc)
  df_fit_f["p_h1"] = df_fit_f.apply(lambda r: solve_logit_h1(r["M"], r["Spend"]), axis=1)
  df_fit_f["p_rat"] = (rc1*df_fit_f["Spend"] + rc2*df_fit_f["M"] + rc3) / (rc4*df_fit_f["Spend"] + rc5*df_fit_f["M"] + 1)

  models = [
      ("Quadratic (Unified)", df_fit_f["p_quad"], 
       lambda l: qa1*l**2 + qa2*l + qa3, lambda l: qb1*l**2 + qb2*l + qb3),
      ("Simple Linear", df_fit_f["p_lin"],
       lambda l: km*l + cm, lambda l: kc*l + cc),
      ("Hyper-Alpha + Lin-Beta", df_fit_f["p_h1"],
       lambda l: 1/(ka*l + ia), lambda l: kb*l + ib),
      ("Direct Rational", df_fit_f["p_rat"],
       lambda l: (rc2 - l*rc5) / (l*rc4 - rc1), lambda l: (rc3 - l) / (l*rc4 - rc1))
  ]
  ...
  for name, pred, a_func, b_func in models:
    ...
    if name == "Quadratic (Unified)":
        ...
        print(f"logit(P) ≈ ((M * {-cm*100:.2f}% + {-cc:.1f}) - Spend) / (M * {km:.4f} + {kc:.2f})")
        ...
        print(f"logit(P) = ({rc1:.8f}*Spend + {rc2:.8f}*M + {rc3:.8f}) / ({rc4:.8f}*Spend + {rc5:.8e}*M + 1)")
        print(f"Spend = (M * ({rc2:.8f} - L * {rc5:.8e}) + ({rc3:.8f} - L)) / (L * {rc4:.8f} - {rc1:.8f})")
        print(f"M = (Spend * ({rc1:.8f} - L * {rc4:.8f}) + {rc3:.8f} - L) / (L * {rc5:.8e} - {rc2:.8f})")
    ...
    for p in [0.98, 0.97, 0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.60]:
      ...
      print(f"| {p*100:g}% | {ta*100:.2f}% | {a_func(lp)*100:.2f}% | {tb:.1f} | {b_func(lp):.1f} |")
