"""
オルカンと無リスク資産（固定利回り 4%）の組み合わせによる生存確率への影響を比較するシミュレーション。

実験1: オルカンを先に使う (Spend Stock First)
実験2: 無リスク資産を先に使う (Spend Risk-Free Asset First)

設定:
- 初期資産: 1億円 (10,000万円)
- 投資先: オルカン (年率 7%, リスク 15%) + 為替リスク (0%, 10.53%)
- 無リスク資産: 利回り 4% (税引後 3.18%)
- 取り崩し額: 毎年400万円 (物価連動)
- インフレ率: 年率 1.77% (固定)
- 譲渡所得税: 20.315%
- 信託報酬: 0.05775%
...
def main():
  ...
  def run_experiment(exp_name: str, exp_title: str, ratios: List[float],
    ...
    for ratio in ratios:
      ...
      name = f"オ:無={round(stock_ratio*100)}:{round(rf_ratio*100)}"
    ...
    # 3. 可視化と保存
    img_dir = "docs/imgs/zero_risk"
    data_dir = "docs/data/zero_risk"
    ...
    visualize_and_save(results=results,
                       ...
                       title=f'無リスク資産併用による生存確率の比較 ({exp_title})',
  ...
  # 比率の設定 (100:0 から 30:70 まで)
  ratios = [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3]

  # 実験1: オルカンを先に使う
  run_experiment(exp_name="exp1",
                 exp_title="実験1: オルカンを先に使う",
                 ratios=ratios,
                 selling_priority_func=lambda s, r: [s, r])

  # 実験2: 無リスク資産を先に使う
  run_experiment(exp_name="exp2",
                 exp_title="実験2: 無リスク資産を先に使う",
                 ratios=ratios,
                 selling_priority_func=lambda s, r: [r, s])
