"""
data/single_33yr/ の結果を分析・可ビジュアル化するスクリプト（単身世帯版）。
...
# 設定
IMG_DIR = "docs/imgs/single_33yr"
DATA_OUT_DIR = "docs/data/single_33yr"
NUM_YEARS = 62
START_AGE = 33
...
def run_optimal_pension_analysis(df_all: pd.DataFrame, target_year: str):
  ...
  # 1. グラフ作成 (m=1, r=3% と m=1.5, r=2%)
  create_pension_survival_curve(df_all,
                                multiplier=1.0,
                                rule=3.0,
                                title="受給開始年齢別 生存確率推移 (支出レベル1.0, 初年度支出率3%)",
                                output_path=os.path.join(
                                    IMG_DIR,
                                    "survival_curve_pension_m1_r3.svg"),
                                start_age=START_AGE,
                                num_years=NUM_YEARS)
  ...
  create_pension_survival_curve(df_all,
                                multiplier=1.5,
                                rule=2.0,
                                title="受給開始年齢別 生存確率推移 (支出レベル1.5, 初年度支出率2%)",
                                output_path=os.path.join(
                                    IMG_DIR,
                                    "survival_curve_pension_m1_5_r2.svg"),
                                start_age=START_AGE,
                                num_years=NUM_YEARS)
...
def main():
  parser = argparse.ArgumentParser(
      description="33歳リタイア開始・95歳までの分析・可視化スクリプト（単身世帯版）。")
  ...
  for et in exp_types:
    et = et.strip()
    csv_path = f"data/single_33yr/{et}.csv"
