"""
オルカンと現金の比率による4%ルールの生存確率への影響を比較するシミュレーション。
"""
...
def main():
  ...
  visualize_and_save(results=results,
                     ...
                     title='資産配分（オルカン vs 現金）による生存確率の比較',
                     summary_title='資産配分比率の影響サマリー（1,000回試行）',
  ...
  try:
    ...
    pattern = r'(<!--<cash_ratio_comp_main\.py>-->).*?(<!--</cash_ratio_comp_main\.py>-->)'
    if re.search(pattern, content, re.DOTALL):
      ...
      print(
          f"\033[91mWarning: Placeholder <cash_ratio_comp_main.py> not found in {report_file}\033[0m",
          file=sys.stderr)
