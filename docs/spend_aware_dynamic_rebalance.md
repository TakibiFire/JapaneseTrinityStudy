...
「94歳開始時点の総資産が $X_94$ の時にその資産の $A_94$ %をオルカンに割り当てると生存できる確率は何％か」を様々な $X_94$, $A_94$ に対して1年分だけシミュレーションを行います。そうすると 94歳時の状況に応じた最適な割り当て戦略や、その戦略を取った後の生存確率が求まります。(厳密には物価上昇率が絡みますが、付録を参照して下さい)。

**その次に「93歳開始時点にどうありたいか」を計算します。**

「93歳開始時点の総資産が $X_93$ の時にその資産の $A_93$ %をオルカンに割り当てると生存できる確率は何％か」を計算したいわけです。様々な $X_93$, $A_93$ に対して1年分だけシミュレーションを行うと、94歳開始時の資産状況がわかります。それがわかれば、先程の「94歳時の状況」の計算結果を使って生存確率が求まります。これにより、93歳時の状況に応じた最適な割り当て戦略や、その戦略を取った後の生存確率が求まります。
...
2.  **年齢ごとの再帰的な計算:**
    ...
    ここで、$\text{CPI}_{93}$ はシミュレーションの各パスが現在経験している固有の物価です。インフレが激しいパスでは、この値が高くなります。$\text{CPI}_{94}$ は来年の物価ですが、これは確定的ではありません。そこで、標準正規分布を代表する7つのZスコア（-3, -2, -1, 0, 1, 2, 3）に対応する7つのインフレシナリオ $\text{CPI}_{94, j}$ を生成します。

    さらに、モデルで説明しきれない現実の支出のブレを反映するため、単なるノイズではなく、**回帰分析の実際の残差分布から正規分布の確率（0.13%, 2.27%, ..., 99.8%）に対応する経験的なパーセンタイル値 (Residual)** を抽出して加算します。

    これらを用いて、7つのシナリオそれぞれにおける予測生存確率を計算し、正規分布の重み $W_j$ を掛けて合算します。

    $$P_{93}(R) = \frac{1}{2000} \sum_{i=1}^{2000} \sum_{j=1}^{7} W_j \times P_{94} (Y_{94,i,j} / X_{94,i})$$

    このように「インフレの不確実性」と「支出のブレ」の両方を織り込むことで、「その1年を生き延びた後、さらに寿命（94歳末）まで生存する確率」がより堅牢に導かれます。
