...
!!! note "相関に関する仮定"
    ...
    実際に、USD/JPYの月末スポットレートの月次対数利益率と、株価指数の対数利益率との間のピアソン相関係数を計算した結果は以下の通りです。

    * **USD/JPY と S&P 500:** r = 0.0001 （期間: 1980年2月 - 2026年2月、553ヶ月）
    * **USD/JPY と ACWI (オルカン):** r = -0.0185 （期間: 2008年4月 - 2026年2月、215ヶ月）

    これらの値は実質的にゼロであり、世界の株価指数の変動と為替（USD/JPY）の変動には相関がないとする本シミュレーションの仮定を経験的に裏付けています。
...
表を見ると、「1. 為替リスクなし」の場合と比べて「2. ドル円のリスク 10.53%」を考慮した場合、50年破産確率が50.8%から58.2%へと悪化していることが確認できます。また、「5. オルカンリスク18.3%」という単純なボラティリティ加算のケースと「2」がほぼ同じ破産確率になっていることから、為替リスクは「ボラティリティの増加」として生存確率を押し下げていることが分かります。

### 資産分布の比較

![為替リスクの影響（資産分布）](imgs/forex/fx_comp_distribution.svg)

為替リスクを考えると(オレンジ)、グラフが右に寄った（より厳しい状態になった）ことがわかりますね。
...
「1. 為替リスクなし」と「2. ドル円のリスク 10.53%」を比較すると、30年破産確率は24.5%から35.3%へ、50年破産確率にいたっては50.8%から58.2%へと悪化しています。
...
海外の金融商品を取り崩す際、為替の変動は必ずついて回ります。為替リスクは「円高で資産価値が下落する」という損失の危険性として語られがちですが、シミュレーションの観点では「株価のボラティリティに為替のボラティリティが上乗せされ、ポートフォリオ全体の合成リスク（ボラティリティ）を上昇させる」という効果が重要になります。

資産形成期においては、為替によってボラティリティが数%上昇したとしても、長期的な株式の成長リターンでカバーできるため、過度に心配する必要はありません。

参考: [為替リスクとは | 普通の人が資産運用で99点をとる方法とその考え方](https://hayatoito.github.io/2020/investing/#8bda)

しかし、取り崩し期においては状況が異なります。[ボラティリティの影響](volatility.md)で解説したように、ボラティリティの上昇は「ボラティリティ・ドラッグ（リターンの押し下げ）」を引き起こし、生存確率の低下に直結します。

そのため、==取り崩しのシミュレーションを行う際には為替の影響（ボラティリティの加算）を確実に考慮するべきです。== 為替変動を直接組み込めるツールを使うのが理想ですが、もし利用できない場合は、本ページで解説した「株価と為替の合成リスク」の値を全体のボラティリティとして設定してシミュレーションすることをお勧めします。
