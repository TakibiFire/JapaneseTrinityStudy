---
description: 資産残高と残り年数に応じて最適配分を毎年更新する「ダイナミックリバランス」戦略を提案。固定比率や年齢ベースのルールよりも高い生存確率を達成できる理由を、シミュレーション結果と計算ツールを用いて解説します。
---

# 最適配分を繰り返す<wbr/>(ダイナミックリバランス)
...
!!! abstract "重要なポイント"
    ...
    * **最適配分するツールを公開。** ==[最適配分を表示するツールを作った](#ツールを作りました)== ので使ってみてほしい。最適配分はあなたの生存したい残り年数と、年支出率で決まる。
...
!!! info "「限界年数」の意味"
    ...
    実はこの年数は、「無リスク資産100%の割り当てにして（税引き後実質利回り 約3.18% - インフレ率 1.77% = 実質 1.41%）取り崩しを行った場合に、資産が完全に底をつく理論上の年数」のことです。
...
!!! info "ツールの使い方"
    ...
    例えばあなたが50歳で1億円あり、今年の支出が400万だとして、95歳まで生存したい場合、年支出率が 4%、目標寿命を45年と入力します。そうすると、推奨オルカン比率が90.7%と出てきます。それに合わせてオルカンに90.7%、無リスク資産に9.3%を割り当てます。そこから生活費分と譲渡所得税だけを取り崩すのが最適となります。
    ...
    仮に翌年、(税抜き後)資産が伸びて10500万円となり、51歳時の予測支出が410万だとした場合、年支出率 3.9%, 目標寿命を44年を入力します。このように、目標寿命を減らしながら使っていきます。

    推奨オルカン比率が 84.7% なので、そうなるようにリバランスしていきます。

!!! warning "このツールの前提"

    このツールは物価上昇率を無視したあなたの実質支出が固定であることを想定しています。
    実際には人の支出の平均は50代前後をピークとしたグラフを描きます ([「高齢になると支出が減るという話」](retired_spending.md))し、60歳以降は年金保険料の支払いが終わったり、年金収入が入ったりします。
    こういうライフプランを加味するとさらに最適化できるよ、という話を[「ライフプランに基づく動的資産配分」](spend_aware_dynamic_rebalance.md)で解説します。
