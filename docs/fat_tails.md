---
description: 実際の相場で発生する極端な暴落・暴騰「ファットテール」が、FIRE取り崩し戦略に与える影響を検証。高度な分布モデルを用いたシミュレーション結果に基づき、長期の生存確率への影響が限定的である理由を明らかにします。
---
...
<!--
DO NOT DELETE:
python3 src/analyze_fat_tails_main.py
python3 src/fat_tails_comp_main.py
-->
...
!!! abstract "重要なポイント"
    ...
    * **ファットテールの長期的な影響は限定的。** 期待リターン（幾何平均）を過去実績と等しく揃えた公平な比較において、対数正規分布モデルとファットテールモデルの間に、50年後の枯渇確率の有意な差（37.9% vs 37.6%）は見られなかった。
...
* 50年という長期間のシミュレーションにおいて、日単位の微細なモデルのズレが蓄積し、結果の精度が低下するのを防ぐ
* 計算負荷を抑え、シミュレーションの効率を高める
* 実際の資産取り崩しは月単位で行われることが一般的であり、月次リターンの方が現実に即している
...
!!! warning "以前の {リターン7%, リスク15%} とは違う値を使っています"
    ...
    リターンが高く設定されているため、本章での50年破産確率（約40%）は為替の章（約62%）よりも低く（良く）出ていますが、これはファットテールの影響ではなく前提条件（ベースとなるリターンの違い）によるものです。章をまたいで破産確率の絶対値を直接比較しないようご注意ください。オルカンとして一体何の値を使うべきか、S&P500との比較などは[次の回](sp500_vs_acwi.md)で行います。==今回はファットテールの影響のみを見ていきます。==
...
と、**ほとんど差が出ない（わずかに誤差レベルの違いしかない）**ことがわかります。

![資産額の推移（パーセンタイル）](imgs/fat_tails/fat_tails_comp_distribution.svg)

これを見ると、資産の推移もほぼ一致しています。

ファットテールは正規分布ではほぼ起こらないとされる暴落を再現しますが、同様に暴騰も再現します。リターンの中央値（幾何平均）を過去の実績に正しく固定して比較した場合、50年間（600ヶ月）という超長期のシミュレーションにおいては、毎月の極端な変動が相殺され、全体的な生存確率に与える影響は限定的になることがわかります。

![生存確率の推移](imgs/fat_tails/fat_tails_comp_survival.svg)

## 結論

実際の相場に見られる「ファットテール」をシミュレーションに組み込んでも、長期的な期待リターン（幾何平均）を厳密に揃えた公平な比較においては、資産が枯渇する確率は正規分布を前提とした場合とほとんど変わらないことが確認できました。

ファットテールな環境では想定外の大暴落が頻発しますが、そのダメージは同等の大暴騰や、通常時の細かなリターンの積み重ねによって超長期的には吸収されます。50年という長い取り崩しフェーズにおいては、月次レベルのファットテール（分布の裾の太さ）よりも、**「資産そのものが持つ根本的な期待リターンとボラティリティの大きさ」**の方が生存確率に決定的な影響を与えます。

==つまり、適切な期待リターンとボラティリティさえ設定されていれば、対数正規分布を用いたシンプルなシミュレーションでも十分に信頼できる生存確率を計算できると言えます。==
