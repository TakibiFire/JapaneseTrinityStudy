...
!!! abstract "重要なポイント"
    ...
    * **ファットテールを考慮すると、資産の枯渇確率が上昇する。** 大暴騰も起こるものの、長期間の取り崩しにおいては、大暴落によるダメージが大きく、50年後の枯渇確率は47.6%から50.2%へと悪化するなど、結果的に生存確率を押し下げる要因となる。
...
資産運用のシミュレーションにおいては、算術リターン（Simple return）ではなく対数リターン（Log return）を用いることが一般的です。それにはいくつか理由があります。

??? info "1. 加法性が便利"

    対数リターンは長期シミュレーションをする際にただ足し算をするだけでいいので、シミュレーションしやすいです。

??? info "2. 算術リターンの正規分布は負のリターンを作る可能性がある"

    算術リターンは、理論上-100%（資産がゼロになる）より下にはなり得ません。しかし、正規分布は左右対称に無限に広がるため、モデル上は「リターンが-150%」といった負の資産が発生するパスが含まれてしまいます。

    これを防ぐために、実装上でリターンを-99%などで切り捨て（クリッピング）を行うと、分布の左側の裾（大損する可能性）が物理的に削り取られます。その結果、分布全体の確率の重心が本来よりも右側（プラス側）へ押し上げられてしまい、資産の伸びが異常に高くなる原因となります。

    対照的に「対数リターン」を用いるモデルでは、価格が0を下回ることが数学的にあり得ず、かつボラティリティによる減速も計算式の中に自然に織り込まれるため、長期シミュレーションにおいてより正確な予測が可能になります。

??? info "3. 算術リターンの正規分布を使って長期のシミュレーションをすると結果が本来より高くなる"

    長期的な資産形成（複利）のスピードを決定するのは「算術平均（足し算の平均）」ではなく「幾何平均（掛け算の平均＝CAGR）」です。数学的には、幾何平均は算術平均よりも低くなることが知られています（近似式：$\text{幾何平均} \approx \text{算術平均} - \sigma^2/2$）。

    算術リターンの正規分布モデルでは、過去データの「算術平均」をそのまま分布の期待値として設定してしまいます。しかし、ボラティリティ（分散 $\sigma^2$）が存在する限り、実際の資産の成長速度（中央値）はその期待値よりも必ず遅くなります。この「遅れ」を考慮せずにシミュレーションを回すと、現実よりも強力な成長エンジンを積んでいることになり、長期になるほど結果が過大評価されます。
...
!!! warning "以前の {リターン7%, リスク15%} とは違う値を使っています"
    ...
    オルカンとして一体何の値を使うべきか、S&P500との比較などは別の回で行います。今回はファットテールの影響のみを見ていきます。
...
生存確率も全ての経過年数で下がっています。

## 結論

実際の相場に見られる「ファットテール（極端な変動が起きやすい性質）」をシミュレーションに組み込むと、正規分布を前提とした場合と比べて、資産が枯渇する確率がわずかに（数パーセント程度）上昇することが確認できました。
