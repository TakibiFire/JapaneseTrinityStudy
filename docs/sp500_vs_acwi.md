---
description: FIRE取り崩し戦略において、S&P500と全世界株式（オルカン）のどちらが有利かをシミュレーションで検証。歴史的なリターンとボラティリティの差異が、長期的な生存確率にどのような影響を与えるのかを分析します。
---

# S&P500 vs. オルカン ~ <wbr/>取り崩し戦略に差はあるのか？

<!--
DO NOT DELETE:
python3 src/sp500_acwi_comp_main.py
python3 src/analyze_sp500_acwi_main.py
-->

資産形成期において、S&P500と全世界株式（オルカン）のどちらを選ぶべきかという議論はよくありますが、どちらを選んでも大きな差はないと言われます。しかし、資産を取り崩すフェーズにおいては、そのわずかな特性の違いが生存確率に無視できない影響を与える可能性があります。本記事では、歴史的なデータに基づくシミュレーションを通じ、取り崩し戦略における両者の違いを検証します。

!!! abstract "重要なポイント"
    * **モデルの作り方（採用するデータ期間）で結果が大きく変わる。** S&P500とオルカンの比較において最大のポイントは、「どの期間のデータでモデルを作るか」によって結果が大きく変わる点にある。
    * **実績データ上はS&P500が優位に見える。** S&P500は直近30年間のリターンが155年の歴史的平均よりも好調だった。また、オルカンのデータが存在する直近18年間を基準にしても、米国株のパフォーマンスが他国を圧倒しており、オルカンよりもS&P500の方が破産確率が低く（良く）算出される。
    * **結局は「どのシナリオを信じるか」に帰結する。** 将来何が正解になるかは誰にも分からない。米国株の優位性が続くか、長期的なサイクルでの米国低迷（新興国の台頭）に備えるかなど、「実績のあるデータをどう信じるか」という仮説によって選ぶべき資産が変わる。
...
??? question "対数正規分布を使うか、ファットテールを捉えられるモデルを使うか → ファットテールを一応使ってみる"

    株価の対数リターンのデータは、正規分布を想定したときには決して起きなさそうな暴落をたくさん含んでいます。[ファットテールの影響](fat_tails.md)の回で紹介した通り、真面目に計算してもしなくても、それほど生存確率に影響は無かったのですが、今回複雑な近似モデルも扱うので、念のためファットテールを扱えるモデルを使います。
...
| **4. オルカン (S&P近似)** | 155年相当 | モデル1との1次近似<br/> + Dweibull | 7.05% | 15.77% |
...
!!! warning "モデル4の50年破産確率は 61.8% と、他のモデルに比べて悪化しています。"
...
### オルカンの方がボラティリティが高いことの確認

実績データの、各年ごとの算術平均リターンとボラティリティ（標準偏差）の推移を可視化しました。

![年率算術リターンの推移](imgs/sp500_acwi/sp500_acwi_annual_return.svg)

多くの年でS&P500の方がリターンがオルカンを上回っていることがわかります。

![年率算術ボラティリティの推移](imgs/sp500_acwi/sp500_acwi_annual_volatility.svg)

??? info "2008年のデータ取扱い"
    ACWI（オルカン）のデータは2008年4月からデータが開始されているため、2008年は9ヶ月分のみ存在します。本グラフでは、この9ヶ月分の標準偏差に$\sqrt{12}$を乗じて年率換算しています。

グラフを見ると、2016年以前はオルカンの方がボラティリティが高く、2016年以降はだいたい一緒か若干小さい傾向が見られます。平均するとオルカンの方がボラティリティが高かったことがわかります。

シミュレーションでオルカンの破産確率が高くなった背景には、リターンの低さとボラティリティの高さの両方が影響しています。
