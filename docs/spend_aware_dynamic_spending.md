# ライフプランを考慮した支出制限（生存確率ベースのガードレール）
...
* **生存確率が下限（例：85%）を下回った場合**: 計画の維持が難しくなるリスクが高まったと判断し、生存確率を 85% に戻すために必要な支出削減量を算出します。その削減を適用しますが、生活への影響を考慮し、1年間の削減幅は最大 1.0% に制限します（不足分は翌年以降に持ち越されます）。
...
!!! info "共通設定"
    ...
    * **ライフプラン（40歳リタイア想定）**: 
        ...
        * **年金受給**: 60歳から繰り上げ受給を開始 (基礎年金 + 厚生年金 = 約99.4万円/年。マクロ経済スライド考慮)
...
2. **生存確率ベースの動的支出 (今回の手法)**:
    ...
    支出上限を高くしているのは、それがトリガーされる条件が97%生存という高い値だからです。減額には保守的な設定を使っています。
...
すべての初期支出設定において、生存確率ベースの動的支出は支出率ベースの動的支出よりも高い生存確率を維持しています。特に初期支出率が 4.0% 以上の設定において、生存確率の維持能力に大きな差が現れています。これは、本手法が将来の危機をいち早く検知し、手遅れになる前に「予防的な支出削減」を段階的に開始するためです。
...
緑と青の乖離: [以前の考察](dynamic_spending.md#考察生存確率を決定づける目標支出率)でも述べたように、40歳時点で初年度支出率 4.0% というのは非常に厳しいスタートです。シミュレーション内で初年度の予測生存確率は約54%と算出されるため、市場が大きく上振れない限り、この手法は生存確率85%を目指して毎年 1.0% ずつの支出削減（我慢）を強いることになります。

実際、グラフを見ると、60歳（経過年20年）時点でのベースライン（予定取り崩し額）は約540万円ですが、青色の生存確率ベースの中央値（p50）は約440万円、下位25%（p25）は約350万円まで落ち込んでいます。下位25%のケースでは36%もの支出削減を強いられていることになります。これは、初期に設定した 4.0% という支出率が、40歳時点の資産規模に対して過大であったことを示しています。
...
3.0% で開始した場合は安全余裕があります。青の中央線が緑と近く、生存確率ベースの手法でもベースラインに近い取り崩し額を維持しやすくなっていることがわかります。これは、シミュレーション内部での95歳生存確率は初年度からすでに90%以上になっているためで、支出の削減をすぐには必要としないためです。

ただし、青エリアの下側を見ると、運用が悪化した場合などにはきちんと取り崩しを抑えている様子がわかります。暴落や高騰があった場合には支出額を適切に調整していることが、分布の幅から読み取れます。
...
確かに「生存確率ベースの動的支出」は、4.0% という高い支出率から開始しても90%以上の生存確率を達成しています。しかし、これは「資産の枯渇リスク」を「生活水準の低下リスク」へと変換して受け入れていることに他なりません。20年で36%もの削減を強いられるシナリオがある以上、調整ルールに頼る前に初期のゆとりを持つことが重要です。
