...
さらに厳しい「動的支出（上限+2%, 下限-2%）」や「動的支出（上限+2%, 下限0%）」では、破産確率がさらに下がります。市場の変動に合わせて生活費を調整できることが、生存確率を上げる有効な手段なことがわかりますね。
...
この比較を様々な (目標年数, 初年度年支出割合) でやってみた結果、目標年数後の生存確率を計算しました。
