...
このデータを元に、他の値でもオルカン比率を求める近似値を求めるロジックを作ってみました。
...
支出率 ($S$) と目標とする経過年数 ($N$) に応じて、生存確率を最大化する「最適オルカン比率」をシミュレーションした結果、非常に興味深いパターンが見えてきました。

**1. 資産寿命 ($N_{ruin}$) という境界線**

まず、無リスク資産（実質利回り 約3.18%）だけを切り崩していったときに、資産が底をつく理論上の年数を **資産寿命 ($N_{ruin}$)** と呼びます。
...
| 3.00% (33倍) | 約 42.6 年 |
| 2.50% (40倍) | 約 54.6 年 |
| 2.00% (50倍) | 理論上無限 (1.2%のプラス利回りの方が大きいため) |

この $N_{ruin}$ を境にして、最適な戦略は大きく2つのフェーズに分かれます。

* **フェーズ 1 ($N \le N_{ruin}$)**: 無リスク資産だけでも理論上は生存可能な期間。この時期は、リスクを最小限に抑えつつ、運用初期の暴落を回避することが生存確率を高めます。
* **フェーズ 2 ($N > N_{ruin}$)**: 無リスク資産だけでは確実に枯渇する期間。この時期を生き抜くには、オルカンの高い成長力に頼らざるを得ず、リスクを取ってリターンを追求することが「生存のための必須条件」となります。

**2. 最適なオルカン比率を導く「ルール・オブ・サム」**

シミュレーション結果を統計的に解析し、支出率 ($S$) と目標年数 ($N$) から最適なオルカン比率 ($f$) を予測する近似式を導き出しました。

$$n = N / 50$$
$$m = (N - N_{ruin}) / 50 \quad (\text{Region 2 のみ})$$

**【Region 1: $N \le N_{ruin}$ の場合】**
無リスク資産で逃げ切れる可能性があるため、年数が経つほど（$N$が増えるほど）、あるいは支出が多いほど、オルカン比率を下げて守りを固めるのが合理的です。
$$g(S, n) = -0.7967 -0.3815 \ln(n \cdot S) + \frac{0.1160}{n} -0.2093 \ln(S) -1.4521 e^S$$

**【Region 2: $N > N_{ruin}$ の場合】**
資産寿命を超えて生き抜く必要があるため、逆にオルカン比率を上げて「攻め」に転じる必要があります。
$$h(S, m) = +0.7746 +0.1809 \ln(m) - \frac{0.0317}{S} + 0.9758 e^S + 0.0075 \frac{n}{S}$$
*(※ $f$ は $0.0 \le f \le 1.0$ の範囲にクランプします)*
...
* **若い時のリタイアほど「攻め」が必要**: 50年以上の生存を目指す場合、必然的に $N > N_{ruin}$ の期間が長くなります。そのため、初期から高いオルカン比率を維持する覚悟が必要になります。
* **高齢期の安心は「無リスク資産」で作る**: $N < N_{ruin}$ の範囲内に収まるような短・中期の計画（例えばあと15年〜20年持てば良いという状況）であれば、オルカン比率を大胆に下げて無リスク資産に寄せることで、市場の混乱に怯えることなく生活を維持できます。
